...
count_vectors = count_vectorizer.fit_transform(emails_df['processed_text'])
...
count_vectors_dense = count_vectors.toarray()
tfidf_vectors_dense = tfidf_vectors.toarray()

# Apply PCA for dimensionality reduction
pca = PCA(n_components=2)  # Reduce to 2 dimensions for visualization
count_pca = pca.fit_transform(count_vectors_dense)
tfidf_pca = pca.fit_transform(tfidf_vectors_dense)

# Apply KMeans clustering
kmeans_count = KMeans(n_clusters=2, random_state=42)
count_labels = kmeans_count.fit_predict(count_pca)

kmeans_tfidf = KMeans(n_clusters=2, random_state=42)
tfidf_labels = kmeans_tfidf.fit_predict(tfidf_pca)

# Add results to DataFrame
emails_df['kmeans_count_labels'] = count_labels
emails_df['kmeans_tfidf_labels'] = tfidf_labels

# Display Results
print("Processed Emails:\n", emails_df['processed_text'], "\n")
print("KMeans Clustering Results (Count Vectorizer):\n", emails_df[['email_id', 'text', 'kmeans_count_labels']], "\n")
print("KMeans Clustering Results (TF-IDF Vectorizer):\n", emails_df[['email_id', 'text', 'kmeans_tfidf_labels']], "\n")
